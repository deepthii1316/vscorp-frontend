...
function Delta({ value, unavailableText = 'M2M unavailable' }) {
  if (value == null) return <span className="md-metric-delta flat"><Minus />{unavailableText}</span>;
  ...
  return <span className={`md-metric-delta ${tone}`}><Icon />{Math.abs(value).toFixed(1)}% M2M</span>;
}

function MetricBar({ metric, current, previous }) {
  ...
  return (
    <div className="md-metric-bars">
      ...
      <div className="md-metric-bar-row">
        <span>M2M</span><div><i className="previous" style={{ width: `${previousWidth}%` }} /></div><strong>{metric.format(previous)}</strong>
      </div>
    </div>
  );
}

export default function MasterDashboardMetrics({ days, prevDays, ssrDays, ssrPrevDays, rangeText }) {
  ...
  return (
    <div className="md-metrics-view">
      ...
      <div className="card md-metric-chart-card">
        ...
        <p className="md-note">Current period vs M2M for the single Virata Retail store{rangeText ? `, ${rangeText}` : ''}.</p>
        <div className="md-metric-legend"><span><i className="current" />Current period</span><span><i className="previous" />M2M</span></div>
        <div className="md-metric-store-row"><span>Uppal Reebok</span><MetricBar metric={metric} current={current[selectedMetric]} previous={previous[selectedMetric]} /></div>
      </div>

      <div className="card">
        <div className="card-header"><TrendingUp className="card-header-icon-svg" /><h3>Store performance</h3></div>
        <p className="md-note">All seven retail metrics with M2M comparison. Virata currently has one uploaded store.</p>
        <div className="md-table-wrap">
          <table className="md-table md-metric-table">
            ...
            <tbody><tr><td>Uppal Reebok</td>{METRICS.map((item) => {
              ...
              return <td key={item.key}><strong className="md-metric-value-chip" style={{ borderBottomColor: relativeColor(current[item.key], values) }}>{item.format(current[item.key])}</strong><Delta value={delta(current[item.key], previous[item.key])} unavailableText={item.key === 'ssr' ? '—' : undefined} /></td>;
            })}</tr></tbody>
          </table>
        </div>
      </div>
